...
//		P4OUT |= BIT4; 								// Change for science board BIT4: 1; BIT5: 2; BIT6: 3; BIT7: 4;
		P4OUT &= ~(BIT4 + BIT5 + BIT6 + BIT7);				// Change for science board BIT4: 1; BIT5: 2; BIT6: 3; BIT7: 4;
		break;
...
//		P4OUT &= ~BIT1;									// Pulls SYNC low
		...
		while ((UCB0STATW & UCBUSY) && !timeout) {};					// While not busy
//		P4OUT |= BIT1;
	while (!(UCB0IFG & UCRXIFG) && !timeout) {};    				// While RX flag is high
