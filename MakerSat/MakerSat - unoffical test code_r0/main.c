...
//			}
		...
		case IMU_Send:
			if (radio_Send(0)){timeout = false;location = 0x03;IMU_State = IMU_FAIL;break;}	// Send whats in circular buffer
			IMU_State = IMU_Done;
			break;
		...
		case IMU_FAIL:
			...
			radio_Send(0);//){timeout = false;IMU_State = IMU_Done;break;}	// Send whats in circular buffer
			timeout = false;
			IMU_State = IMU_Done;
			location = 0x00;
			break;
		...
		case IMU_Done:
			clear_CircularBuffer();
			LED(1,0,0,5,1); // BLUE
			IMU_mode = false;	// Timer logic
			IMU_done = true;	// Main loop logic
			break;
	}
}
...
///////////////////////////////////
		...
		case SCI_1_PowerDown:
			if(power_Down(1)){timeout = false;location = 0x04;SCI_1_State = SCI_1_FAIL;break;}	// Power up science board 1 = NNU/POLY
			SCI_1_State = SCI_1_Send;
			break;
			
		case SCI_1_Send:
			if (radio_Send(1)){timeout = false;location = 0x05;SCI_1_State = SCI_1_FAIL;break;}		// Send whats in circular buffer
		...
		case SCI_1_FAIL:
			...
			write_Buffer(0xFA); // Fail code
			write_Buffer(location); // Fail location
			radio_Send(1);//){timeout = false;SCI_1_State = SCI_1_Done;break;}	// Send whats in circular buffer
			timeout = false;
			SCI_1_State = SCI_1_Done;
			location = 0x00;
			break;
		...
		case SCI_1_Done:
			clear_CircularBuffer();
			LED(0,1,0,5,1);	// GREEN
			sci_1_mode = false;// Timer logic
			sci_1_done = true;// Main loop logic
			break;
	}
}
...
void SCI_2_Loop()
{
	switch(SCI_2_State){
		case SCI_2_SMStart:
			sci_2_mode = true;// Timer logic
			Start_Timer();
			SCI_2_State = SCI_2_PowerUp;
			break;

		case SCI_2_PowerUp:
			if(power_Up(2)){timeout = false;location = 0x01;SCI_2_State = SCI_2_FAIL;break;};	// Power up science board 2 = Caldwell/RAD
...
//			else if(get_state_x_Send()){power_Down(2);SCI_2_State = SCI_2_Send;break;}
		...
		case SCI_2_Send:
			if(radio_Send(2)){timeout = false;location = 0x05;SCI_2_State = SCI_2_FAIL;break;}	// Send what is in circular buffer
		...
		case SCI_2_FAIL:
			...
			write_Buffer(0xFA); // Fail code
			write_Buffer(location); // Fail location
			radio_Send(2);//){timeout = false;SCI_2_State = SCI_2_Done;break;}	// Send whats in circular buffer
			timeout = false;
			SCI_2_State = SCI_2_Done;
			location = 0x00;
			break;
		...
		case SCI_2_Done:
			clear_CircularBuffer();
			LED(0,0,1,5,1);	// RED
			sci_2_mode = false;// Timer logic
			sci_2_done = true;// Main loop logic
			break;
	}
}
