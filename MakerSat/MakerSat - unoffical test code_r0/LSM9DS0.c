...
uint8_t initGyro(void) {
	if(single_byte_write_i2c(GAddress, CTRL_REG1_G, 0x0F)){return 1;}
	if(single_byte_write_i2c(GAddress, CTRL_REG4_G, 0x30)){return 1;}
	else return 0;
}

uint8_t initXM(void) {
	if(single_byte_write_i2c(XMAddress, CTRL_REG1_XM, 0x67)){return 1;}
	if(single_byte_write_i2c(XMAddress, CTRL_REG2_XM, 0x20)){return 1;}
	if(single_byte_write_i2c(XMAddress, CTRL_REG5_XM, 0xF0)){return 1;}
	if(single_byte_write_i2c(XMAddress, CTRL_REG6_XM, 0x60)){return 1;}
	if(single_byte_write_i2c(XMAddress, CTRL_REG7_XM, 0x00)){return 1;}
	return 0;
}
...
uint8_t run_IMU(void) {

	for (temp = 5; temp > 0; temp--){
		if(read_Gyro()){return 1;}
		if(read_Acc()){return 1;}
		if(read_Mag()){return 1;}
		if(read_Temp()){return 1;}
		write_IMU();
